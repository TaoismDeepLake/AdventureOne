...
public class ComponentTopDungeonRoomBase extends ComponentBase {
    ...
    StructureTopDungeon.Start start; //please note that when populating, this may be null.
    ...
    //grammar
    public ComponentTopDungeonRoomBase() {
        setCoordBaseMode(EnumFacing.SOUTH);
    }

    public ComponentTopDungeonRoomBase(int type, EnumFacing mainDirection, Vec3i vec3i, StructureTopDungeon.Start start) {
        super(type);
        ...
        if (ModConfig.DEBUG_CONF.DEBUG_MODE) {
            StructureTopDungeon.Start.log("[N]Created a room index(%s,%s,%s), bb is %s", vec3i.getX(), vec3i.getY(), vec3i.getZ(), this.boundingBox);
        }
...
//        decorateCorner(worldIn, randomIn, sbb);
    ...
    protected void buildExteriorWallAndClean(World worldIn, Random randomIn, StructureBoundingBox sbb) {
        //basic exterior
//        StructureTopDungeon.Start.log("building walls: %s", relPos);
...
//            StructureBottomDungeon.Start.log("connectionsMicro: %d,%d,%d,%d,%d,%d", doorMin, doorMax, yMin, yMax, depthMin, depthMax);
    ...
    public void connect(EnumFacing facing, ComponentTopDungeonRoomBase target) {
