...
public class StructureTopDungeon extends MapGenStructure {
    public static final String NAME = "TopDungeon";
    ...
    public static class Start extends StructureStart {
        protected HashMap<Vec3i, ComponentTopDungeonRoomBase> roomHashMap = new HashMap<>();
        ...
        {
            basePos = new BlockPos(x << 4, ModConfig.DUNGEON_CONF.TOP_DUNGEON_Y, z << 4);
            ComponentTopDungeonRoomBase firstComp = new ComponentTopDungeonRoomCenter(1, EnumFacing.NORTH, Vec3i.NULL_VECTOR, this);
//            firstComp.connections.put(EnumFacing.UP, EnumConnection.PASS);
        ...
        public ComponentTopDungeonRoomBase getNewRoom(int type, EnumFacing mainDirection, Vec3i vec3i, StructureTopDungeon.Start start) {
            ComponentTopDungeonRoomBase room = new ComponentTopDungeonRoomSide(type, mainDirection, vec3i, start);
            room.buildComponent(room, null, random);
            enlistRoom(room);
            return room;
        }

        boolean expandRoom(ComponentTopDungeonRoomBase roomBase, Random random) {
            cascadeLevel++;
        ...
        ComponentTopDungeonRoomBase goDir(ComponentTopDungeonRoomBase roomBase, Random random, EnumFacing dir) {
            cascadeLevel++;
            log("go dir from %s", roomBase.relPos);
            BlockPos nextPos = new BlockPos(roomBase.relPos).offset(dir);

            ComponentTopDungeonRoomBase targetRoom = roomHashMap.get(nextPos);
            if (targetRoom != null) {
                ...
                //create one
                ComponentTopDungeonRoomBase nextRoom = getNewRoom(0, dir, nextPos, this);
        ...
        public void enlistRoom(ComponentTopDungeonRoomBase roomBase) {
