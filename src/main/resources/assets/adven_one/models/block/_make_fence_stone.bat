...
for %%x in (%*) do (
	echo Making %%x_stone_fence_post.json block
	(
		echo {
	...
	echo Making %%x_stone_fence_side.json block
    	(
    		echo {
    ...
    echo Making %%x_stone_fence_inventory.json block
        	(
        		echo {
	...
	(
		echo {
        echo	"multipart": [
        echo		{   "apply": { "model": "%modid%:%%x_stone_fence_post" }},
        echo		{   "when": { "north": true },
        echo			"apply": { "model": "%modid%:%%x_stone_fence_side", "uvlock": true }
        echo		},
        echo		{   "when": { "east": true },
        echo			"apply": { "model": "%modid%:%%x_stone_fence_side", "y": 90, "uvlock": true }
        echo		},
        echo		{   "when": { "south": true },
        echo			"apply": { "model": "%modid%:%%x_stone_fence_side", "y": 180, "uvlock": true }
        echo		},
        echo		{   "when": { "west": true },
        echo			"apply": { "model": "%modid%:%%x_stone_fence_side", "y": 270, "uvlock": true }
        echo		}
        echo	]
        echo }
	) > ../../blockstates/%%x_stone_fence.json
)